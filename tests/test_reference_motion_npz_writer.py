...
def test_reference_motion_npz_writer_module_has_no_assembly_runner_surface() -> None:
    import mjlab_playground.motion_lib.reference_motion_npz_writer as writer_module

    assert writer_module.ReferenceMotionNpzWriter is not None
    assert not hasattr(writer_module, "package_reference_motions")
    assert not hasattr(writer_module, "parse_args")
    assert not hasattr(writer_module, "main")


def test_removed_writer_module_direct_invocation_fails_with_runner_direction() -> None:
